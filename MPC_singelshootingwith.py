...
for k in range(N):
    v_des_val = U[0, k]  # Get v_des from U
    omega_des_val = U[1, k]
...
def shift(T, t0, x0, u, f, v_current, omega_current):
    ...
    if isinstance(f_value, SX) or isinstance(f_value, MX):
        f_value = DM(f_value).full()  # 确保 f_value 是数值类型
...
u0 = np.zeros((N, 2))  # Initialize control inputs to zero
...
while norm_2(x0 - xs) > 1e-2 and mpciter < sim_tim / T:
    ...
    t0, x0, u0, v_current, omega_current = shift(T, t0, x0, u, f, v_current, omega_current)
