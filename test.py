...
args = {
    'lbg': ca.DM.zeros((n_states*(N+1), 1)),  # 状态 x 和 y 的下界
    'ubg': ca.DM.zeros((n_states*(N+1), 1)),   # 状态 x 和 y 的上界
    'lbx': [None]*(3*(N+1) + 3*N),  # 初始化为 2*N 的列表
    'ubx': [None]*(3*(N+1) + 3*N),
    'p': np.zeros(n_params)
}
...
for i in range(3*(N+1) + 2*N,3*(N+1) + 3*N):
    args['lbx'][i] = 0
    args['ubx'][i] = np.inf
...
Z0 = ca.DM.zeros((1, N))  # initial control
...
if __name__ == '__main__':
    ...
    while (ca.norm_2(state_init - state_target) > 1e-1) and (mpc_iter * T < sim_time):
        ...
        for k in range(1, N + 1):
            ...
            # 提取这一时间步的状态和控制量
            s = 1.5 * s + 1.1 * Z0[k - 1]
            s = fmax(fmin(s, 1.0), 0.0)  # limit s
            print("最最最最好奇的s值是:", s)

            # 使用插值函数计算目标位置
            x_ref = float(spline_x(s))
            y_ref = float(spline_y(s))
            theta_ref = float(spline_theta(s))
        ...
        Z0 = ca.reshape(sol['x'][n_states * (N + 1) + n_controls * N:], 1, N)
        print('增加步伐', Z0)
